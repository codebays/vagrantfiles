...
# 更新YUM源和系统
version="buster"
echo "deb http://mirrors.aliyun.com/debian/ ${version} main non-free contrib" > /etc/apt/sources.list  \
    && echo "deb http://mirrors.aliyun.com/debian/ ${version}-updates main non-free contrib" >> /etc/apt/sources.list \
    && echo "deb http://mirrors.aliyun.com/debian/ ${version}-backports main non-free contrib" >> /etc/apt/sources.list \
    && echo "deb-src http://mirrors.aliyun.com/debian/ ${version} main non-free contrib" >> /etc/apt/sources.list \
    && echo "deb-src http://mirrors.aliyun.com/debian/ ${version}-updates main non-free contrib" >> /etc/apt/sources.list \
    && echo "deb-src http://mirrors.aliyun.com/debian/ ${version}-backports main non-free contrib" >> /etc/apt/sources.list \
    && echo "deb http://mirrors.aliyun.com/debian-security ${version}/updates main" >> /etc/apt/sources.list \
    && echo "deb-src http://mirrors.aliyun.com/debian-security ${version}/updates main" >> /etc/apt/sources.list \
...
if [ -f /usr/lib/systemd/system/rc-local.service ];then
    ...
    } | tee -a /usr/lib/systemd/system/rc-local.service \
    && { \
        echo '#!/bin/sh'; \
...
apt-get install -y apt-transport-https ca-certificates curl gnupg2 software-properties-common > /dev/null \
    ...
    && add-apt-repository "deb [arch=amd64] http://mirrors.aliyun.com/docker-ce/linux/debian ${version} stable" > /dev/null \
    && apt-get update -y > /dev/null \
